...
const w = dom.window;
let deliverRatings;
w.chrome = { storage:{local:{get(k,cb){cb({});},set(){}}}, runtime:{sendMessage(m,cb){deliverRatings=cb;}} };
w.eval(source);
...
assert.match(w.document.querySelector('#zju-helper-switch-modal').textContent,/教师信息未返回/);
classes[0].teacher='测试教师';
send([classes[0]]);
await settle();
assert.match(w.document.querySelector('#zju-helper-switch-modal').textContent,/测试教师/);
assert.equal(w.document.querySelector('.zju-helper-modal-rating'),null);
deliverRatings({ok:true,teachers:[{id:'t1',name:'测试教师',score:'9.2'}]});
await settle();
assert.equal(w.document.querySelector('.zju-helper-modal-rating').textContent,'9.2');
assert.match(w.document.querySelector('.zju-helper-modal-rating').href,/teacherId=t1/);

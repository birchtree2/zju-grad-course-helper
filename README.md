...
- 如果页面请求在扩展初始化前已经完成，扩展会通过初始化握手重放已捕获的课程上下文；同时兼容站点通过 XHR 或 Fetch 发起的课程查询。
- 已选状态沿用 0.6.4 的课程接口数据来源，支持已选、正在修读及待筛选状态。
- 人数刷新采用分阶段策略：已选课程优先并行更新，其余教学班在后台串行查询，不阻塞已选班人数显示；弹窗数据会随着后台查询逐步补齐。
...
完整运行测试：`npm ci && npm test`。包含模拟页面中执行脚本的回归测试，覆盖已选/正在修读/待筛选、多志愿、退课及教学班弹窗数据更新。
